...
import { ChevronLeft, ChevronRight, Minus, Plus, MoveVertical, MoveHorizontal, Maximize2, X, Download, BookOpen, Search, Highlighter, Pencil, Eraser, Undo2, Strikethrough, Volume2, VolumeX, } from "lucide-react";
...
export default function PdfReader({ url, watermark, storageKey, initialPage, onProgress, downloadEnabled, bookId }) {
    ...
    const [highlightedTextIndex, setHighlightedTextIndex] = useState(null);
    ...
    useEffect(() => {
        if (!bookId)
            return;
        fetchAnnotations({ data: { book_id: bookId } }).then((data) => {
            const map = {};
            data.forEach((ann) => {
    ...
    const applyAnnotations = useCallback((pageNum) => {
    ...
    useEffect(() => {
        if (!onProgress || !numPages || !restored)
            return;
    ...
    useEffect(() => {
        if (!storageKey || !numPages)
            return;
        try {
            localStorage.setItem(`plenitude:leitura:${storageKey}`, JSON.stringify({ page, mode, scale }));
        }
        catch {
            /* ignore */
        }
    ...
    useEffect(() => {
        if (mode !== "vertical" || !numPages || !restored)
            return;
        ...
        const observer = new IntersectionObserver((entries) => {
            ...
            if (visible) {
                const n = Number(visible.target.dataset["page"]);
                if (n)
                    setPage(n);
            }
    ...
    const prevResult = () => {
        ...
        if (prevMatch)
            goTo(prevMatch.pageIndex);
    };
    const readCurrentPage = useCallback(async () => {
        if (!pdfInstance.current || numPages === 0)
            return;
        try {
            ...
            speakTextWithHighlight(pageText, (charIndex) => {
    ...
    const textRenderer = useMemo(() => {
        return (textItem) => {
            ...
            return parts.map((part, i) => regex.test(part) ? (<mark key={i} className="bg-yellow-300 text-black px-0.5 rounded-sm">
            {part}
          </mark>) : part);
        };
    ...
    const startDrawing = (e, pageNum) => {
        ...
        const x = ('touches' in e && e.touches[0]) ? e.touches[0].clientX - rect.left : e.clientX - rect.left;
        const y = ('touches' in e && e.touches[0]) ? e.touches[0].clientY - rect.top : e.clientY - rect.top;
        const ctx = canvas.getContext('2d');
        if (!ctx)
            return;
        ctx.beginPath();
    ...
    const draw = (e, pageNum) => {
        if (!isDrawing.current || tool === "none")
            return;
        const canvas = canvasRefs.current[pageNum];
        if (!canvas)
            return;
    ...
    return (<div className={full
      ...
      {full ? (<div className="flex shrink-0 items-center justify-between gap-2 border-b border-border/60 bg-card px-3 py-2">
          ...
          <Button variant="outline" size="icon" className="h-8 w-8 shrink-0" aria-label="Sair da tela cheia" onClick={() => setFull(false)}>
            <X className="h-4 w-4"/>
          </Button>
        </div>) : null}
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border/60 px-2 py-2 sm:px-3">
        <div className="flex min-w-0 items-center gap-1.5">
          <Button variant="outline" size="icon" className="h-8 w-8 shrink-0" aria-label="Página anterior" disabled={page <= 1} onClick={() => goTo(page - 1)}>
            <ChevronLeft className="h-4 w-4"/>
          </Button>
          <span className="shrink-0 text-xs text-muted-foreground sm:text-sm">
            {numPages ? `${page} / ${numPages}` : "..."}
          </span>
          <Button variant="outline" size="icon" className="h-8 w-8 shrink-0" aria-label="Próxima página" disabled={numPages === 0 || page >= numPages} onClick={() => goTo(page + 1)}>
        ...
        <div className="flex min-w-0 flex-wrap items-center gap-1.5">
          <div className="flex shrink-0 items-center gap-1 rounded-md border border-border/60 p-0.5">
            <Button variant={mode === "vertical" ? "default" : "ghost"} size="sm" className="h-8 px-2" aria-label="Leitura vertical" onClick={() => setMode("vertical")}>
              <MoveVertical className="h-4 w-4 sm:mr-1"/>
              <span className="hidden sm:inline">Vertical</span>
            </Button>
            <Button variant={mode === "horizontal" ? "default" : "ghost"} size="sm" className="h-8 px-2" aria-label="Leitura horizontal" onClick={() => setMode("horizontal")}>
          ...
          <div className="flex shrink-0 items-center gap-1 rounded-md border border-border/60 p-0.5">
            <Button variant={tool === "none" ? "default" : "ghost"} size="sm" className="h-8 px-2" onClick={() => setTool("none")} title="Seleção/Navegação">
              <BookOpen className="h-4 w-4"/>
            </Button>
            <Button variant={tool === "pen" ? "default" : "ghost"} size="sm" className="h-8 px-2" onClick={() => setTool("pen")} title="Caneta">
              <Pencil className="h-4 w-4"/>
            </Button>
            <Button variant={tool === "highlighter" ? "default" : "ghost"} size="sm" className="h-8 px-2" onClick={() => setTool("highlighter")} title="Marca-texto">
              <Highlighter className="h-4 w-4"/>
            </Button>
            <Button variant={tool === "strikethrough" ? "default" : "ghost"} size="sm" className="h-8 px-2" onClick={() => setTool("strikethrough")} title="Tachado (Riscar)">
              <Strikethrough className="h-4 w-4"/>
            </Button>
            <Button variant={tool === "eraser" ? "default" : "ghost"} size="sm" className="h-8 px-2" onClick={() => setTool("eraser")} title="Borracha">
              <Eraser className="h-4 w-4"/>
            </Button>
            {tool !== "none" && (<div className="flex items-center gap-1 px-1 border-l border-border/60 ml-1">
                <input type="color" value={penColor} onChange={(e) => setPenColor(e.target.value)} className="w-5 h-5 rounded cursor-pointer bg-transparent border-none p-0" title="Cor da ferramenta"/>
                <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground" onClick={() => clearCanvas(page)} title="Limpar anotações desta página">
          ...
          <div className="flex items-center gap-1.5 border-l border-border/60 pl-1.5">
            <div className="relative flex items-center">
              ...
              <Input placeholder="Buscar na obra..." className="h-8 w-32 pl-7 pr-2 text-xs focus-visible:ring-primary/50 sm:w-48" value={searchTerm} onChange={(e) => handleSearch(e.target.value)}/>
              {searchResults.length > 0 && (<div className="absolute right-2 flex items-center gap-1">
                  <span className="text-[10px] text-muted-foreground">
                    {currentResultIndex + 1}/{searchResults.length}
                  </span>
                  <div className="flex flex-col">
                    <button onClick={prevResult} className="text-muted-foreground hover:text-foreground" aria-label="Resultado anterior">
                      <ChevronLeft className="h-2.5 w-2.5 rotate-90"/>
                    </button>
                    <button onClick={nextResult} className="text-muted-foreground hover:text-foreground" aria-label="Próximo resultado">
                      <ChevronLeft className="h-2.5 w-2.5 -rotate-90"/>
                    </button>
                  </div>
                </div>)}
          ...
          <Button variant="outline" size="icon" className="h-8 w-8 shrink-0" aria-label="Reduzir zoom" onClick={() => setScale((s) => Math.max(0.6, +(s - 0.2).toFixed(2)))}>
            <Minus className="h-4 w-4"/>
          </Button>
          <span className="w-10 shrink-0 text-center text-xs text-muted-foreground sm:text-sm">
            {Math.round(scale * 100)}%
          </span>
          <Button variant="outline" size="icon" className="h-8 w-8 shrink-0" aria-label="Ampliar zoom" onClick={() => setScale((s) => Math.min(2.4, +(s + 0.2).toFixed(2)))}>
            <Plus className="h-4 w-4"/>
          </Button>

          <Button variant={full ? "default" : "outline"} size="sm" className="h-8 shrink-0 px-2" aria-label={full ? "Sair da tela cheia" : "Ler em tela cheia"} onClick={() => setFull((v) => !v)}>
            ...
            {downloadEnabled && (<Button variant="outline" size="icon" className="h-8 w-8 shrink-0 text-green-500 hover:text-green-600 hover:bg-green-500/10 border-green-500/30" aria-label="Baixar obra" onClick={() => window.open(url, "_blank", "noopener,noreferrer")}>
                <Download className="h-4 w-4"/>
              </Button>)}
        </div>
        
        <div className="flex items-center gap-1.5 border-l border-border/60 pl-2">
          {isReadingSequence ? (<Button variant="default" size="sm" className="h-8 bg-red-500 hover:bg-red-600 text-white animate-pulse" onClick={stopReading} title="Parar leitura por voz">
              <VolumeX className="h-4 w-4 mr-1.5"/>
              <span>Parar Ouvir</span>
            </Button>) : (<Button variant="outline" size="sm" className="h-8 border-primary/50 text-primary hover:bg-primary/10" onClick={readCurrentPage} title="Ouvir esta obra (TTS)" disabled={numPages === 0}>
      ...
      {resumedFrom ? (<div className="flex flex-wrap items-center justify-between gap-2 border-b border-border/60 bg-accent/40 px-3 py-2 text-xs text-foreground">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" size="icon" className="h-7 w-7" aria-label="Fechar aviso" onClick={() => setResumedFrom(null)}>
      ...
      <div ref={containerRef} className={`relative w-full max-w-full overflow-auto p-2 sm:p-3 ${full ? "flex-1 min-h-0" : "max-h-[80vh]"}`}>
        <Document file={url} onLoadSuccess={onLoad} loading={<div className="flex flex-col items-center justify-center py-16 animate-in fade-in duration-500">
            ...
            </div>} error={<p className="p-8 text-sm text-destructive">Não foi possível abrir a obra.</p>} className={mode === "vertical"
            ? "flex flex-col items-center gap-4"
            : "flex snap-x snap-mandatory items-start gap-4 overflow-x-auto"}>
          {mode === "vertical" ? (Array.from({ length: numPages }, (_, i) => i + 1).map((n) => (<div key={n} data-page={n} ref={(el) => {
                ...
                {isReadingSequence && page === n && highlightedTextIndex && (<div className="absolute inset-0 z-50 pointer-events-none bg-primary/5 mix-blend-multiply flex items-center justify-center">
                ...
                <Page pageNumber={n} width={width} scale={scale} customTextRenderer={textRenderer} renderTextLayer={true} renderAnnotationLayer={false} className="select-none"/>
                <canvas ref={(el) => {
                canvasRefs.current[n] = el;
            }} width={width * scale} height={(width * 1.41) * scale} // A4 ratio 1:1.41
         className={`absolute inset-0 z-10 ${tool === 'none' ? 'pointer-events-none' : 'cursor-crosshair'}`} onMouseDown={(e) => startDrawing(e, n)} onMouseMove={(e) => draw(e, n)} onMouseUp={() => stopDrawing(n)} onMouseLeave={() => stopDrawing(n)} onTouchStart={(e) => startDrawing(e, n)} onTouchMove={(e) => draw(e, n)} onTouchEnd={() => stopDrawing(n)}/>
              </div>))) : (<div className={`relative mx-auto snap-center shadow-sm ${highContrast ? 'invert hue-rotate-180' : ''}`}>

              <Page pageNumber={page} width={width} scale={scale} customTextRenderer={textRenderer} renderTextLayer={true} renderAnnotationLayer={false} className="select-none"/>
              <canvas ref={(el) => {
                canvasRefs.current[page] = el;
            }} width={width * scale} height={(width * 1.41) * scale} className={`absolute inset-0 z-10 ${tool === 'none' ? 'pointer-events-none' : 'cursor-crosshair'}`} onMouseDown={(e) => startDrawing(e, page)} onMouseMove={(e) => draw(e, page)} onMouseUp={() => stopDrawing(page)} onMouseLeave={() => stopDrawing(page)} onTouchStart={(e) => startDrawing(e, page)} onTouchMove={(e) => draw(e, page)} onTouchEnd={() => stopDrawing(page)}/>
            </div>)}
        </Document>

        {watermark ? (<div className="pointer-events-none sticky inset-x-0 bottom-1/2 flex items-center justify-center">
